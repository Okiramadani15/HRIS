...
class HrPayslip(models.Model):
    ...
    def get_worked_day_lines(self, contracts, date_from, date_to):
        ...
        for contract in contracts.filtered(
            ...
            # Support for the hr_public_holidays module.
            contract = contract.with_context(
                employee_id=contract.employee_id.id, exclude_public_holidays=True
            )
            # only use payslip day_from if it's greather than contract start date
            if day_from < day_contract_start:
                day_from = day_contract_start
    ...
    def _compute_worked_days(self, contract, day_from, day_to):
        ...
        # FIX: Tambahkan pengecekan KeyError untuk menghindari crash jika data employee tidak ditemukan
        employee_id = contract.employee_id.id
        if employee_id in work_data:
            num_days = work_data[employee_id].get("days", 0.0)
            num_hours = work_data[employee_id].get("hours", 0.0)
        else:
            num_days = 0.0
            num_hours = 0.0
            _logger.warning("No work data found for employee %s between %s and %s", contract.employee_id.name, day_from, day_to)
    ...
    def onchange_struct_id(self):
        for payslip in self:
            ...
            payslip.input_line_ids = [(5, 0, 0)] + [(0, 0, r) for r in input_line_ids]
    ...
    def onchange_dates(self):
        for payslip in self:
            ...
            payslip.worked_days_line_ids = [(5, 0, 0)] + [(0, 0, line) for line in worked_days_line_ids]
    ...
    def _compute_name(self):
        for record in self:
            if not record.employee_id or not record.date_from:
                continue
            date_formatted = babel.dates.format_date(
                date=datetime.combine(record.date_from, time.min),
                format="MMMM-y",
